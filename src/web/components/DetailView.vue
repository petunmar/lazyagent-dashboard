...
import { displaySessionName, eventBody, eventImageSrc, eventTitle, relativeTime, shortId } from "../utils";
    ...
    <section class="detail-layout">
      ...
      <section class="console-card transcript-card inline-transcript">
        ...
        <div class="transcript-body" aria-live="polite">
          ...
          <section class="timeline-block raw-events"><h3>Raw session events</h3><article v-for="event in state.rawEvents?.events || []" :key="`${event.kind}-${event.line}`" class="raw-event" :class="event.kind"><header><strong>{{ eventTitle(event) }}</strong><span>{{ event.timestamp ? `${relativeTime(event.timestamp)} · ` : '' }}line {{ event.line ?? '?' }}</span></header><pre v-if="eventBody(event)">{{ eventBody(event) }}</pre><div v-if="event.images?.length" class="event-images"><figure v-for="(image, imageIndex) in event.images" :key="`${event.line}-image-${imageIndex}`"><a :href="eventImageSrc(image)" target="_blank" rel="noreferrer"><img :src="eventImageSrc(image)" :alt="image.path ? `Screenshot ${image.path}` : 'Agent image attachment'" loading="lazy" /></a><figcaption v-if="image.path">{{ image.path }}</figcaption></figure></div></article><p v-if="!state.rawEvents" class="empty">Reading pi JSONL transcript from disk…</p></section>
        </div>
