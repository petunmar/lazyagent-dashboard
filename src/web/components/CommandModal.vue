...
async function submitAgent() { await props.monitor.sendAgent({ mode: isMessage.value ? "message" : "start", ...form }); form.prompt = ""; }
function queueAgent() { if (props.monitor.queueAgentMessage({ cwd: form.cwd, sessionId: form.sessionId, prompt: form.prompt })) { form.prompt = ""; props.monitor.closeModal(); } }
  ...
  <div class="modal-backdrop" @click="closeOnBackdrop">
    ...
    <section v-else class="modal-card" role="dialog" aria-modal="true" :aria-label="isMessage ? 'Message agent' : 'Launch new agent'">
      ...
      <form class="agent-form" @submit.prevent="submitAgent">
        ...
        <label>Session ID<input v-model="form.sessionId" placeholder="empty starts a new session" :readonly="isMessage" /></label><label class="prompt-field">Message<textarea v-model="form.prompt" rows="5" :placeholder="`→ ${isMessage ? `send a message to ${selected ? displaySessionName(selected, state.sessionNames) : 'agent'}` : 'tell the new agent what to do'}`"></textarea></label>
