...
<template>
  ...
  <section v-if="lowFocusSessions.length" class="low-focus-panel console-card" aria-label="Inactive recent sessions">
    ...
    <div class="low-focus-list">
      <article v-for="session in lowFocusSessions" :key="session.session_id" class="low-focus-row" tabindex="0" role="button" @click="monitor.selectSession(session.session_id)">
